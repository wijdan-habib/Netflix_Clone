...
export const getTrendingMovie = async (req, res) => {
  try {
    ...
    const data = await fetchTmdbApi('https://api.themoviedb.org/3/movie/popular?language=en-US&page=1');
...
export const GetMovieTrailers = async function (req, res) {
  const { id } = req.params;  // Extract movie ID from request params

  try {
    // Correct the URL to fetch movie trailers
    const data = await fetchTmdbApi(`https://api.themoviedb.org/3/movie/${id}/videos?language=en-US`);
...
export const GetMovieDetails = async (req, res) => {
  const { id } = req.params;
  try {
    const data = await fetchTmdbApi(`https://api.themoviedb.org/3/movie/${id}?language=en-US`)
  ...
  export const GetSimilarsMovies = async (req, res) => {
    const { id } = req.params;
      try {
        const data = await fetchTmdbApi(`https://api.themoviedb.org/3/movie/${id}/similar?language=en-US&page=1`)
...
export const GetMoviesByCategories = async (req, res) => {
  ...
  try {
    const data = await fetchTmdbApi(`https://api.themoviedb.org/3/movie/${categories}?language=en-US&page=1`);
